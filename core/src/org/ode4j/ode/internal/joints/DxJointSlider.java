...
import static org.ode4j.ode.OdeMath.dCROSS;
...
/**
 * **************************************************************************
 * slider slider. if body2 is 0 then qrel is the absolute rotation of body1 and
...
public class DxJointSlider extends DxJoint implements DSliderJoint {
	...
	public void dJointAddSliderForce(double force) {
		...
		if (node[1].body != null)
			node[1].body
					.dBodyAddForce(-axis.get0(), -axis.get1(), -axis.get2());

		if (node[0].body != null && node[1].body != null) {
			// linear torque decoupling:
			// we have to compensate the torque, that this slider force may
			// generate
			// if body centers are not aligned along the slider axis

			DVector3 ltd = new DVector3(); // Linear Torque Decoupling vector (a
											// torque)

			DVector3 c = new DVector3();
			// c.v[0] = 0.5 * ( joint.node[1].body._posr.pos.v[0] -
			// joint.node[0].body._posr.pos.v[0] );
			// c.v[1] = 0.5 * ( joint.node[1].body._posr.pos.v[1] -
			// joint.node[0].body._posr.pos.v[1] );
			// c.v[2] = 0.5 * ( joint.node[1].body._posr.pos.v[2] -
			// joint.node[0].body._posr.pos.v[2] );
			c.eqDiff(node[1].body.posr().pos(), node[0].body.posr().pos())
					.scale(0.5);
	...
	public double dJointGetSliderPosition() {
		...
		if (node[1].body != null) {
			...
			if (isFlagsReverse()) { // N.B. it could have been simplier to only
									// inverse the sign of
									// the dDot result but this case is
									// exceptional and doing
									// the check for all case can decrease the
									// performance.
									// ax1.v[0] = -ax1.v[0];
				// ax1.v[1] = -ax1.v[1];
				// ax1.v[2] = -ax1.v[2];
	...
	public double dJointGetSliderPositionRate() {
		...
		if (node[1].body != null) {
			return dDOT(ax1, node[0].body.lvel) - dDOT(ax1, node[1].body.lvel);
		} else {
	...
	// void dJointSetSliderParam ( dJoint j, D_PARAM_NAMES parameter, double
	// value )
	...
	public void getInfo2(DxJoint.Info2 info) {
		...
		DVector3 ax1 = new DVector3(); // joint axis in global coordinates (unit
										// length)
	...
	void dJointSetSliderAxisDelta(double x, double y, double z, double dx,
			double dy, double dz) {
		setAxes(x, y, z, axis1, null);
