...
import static org.ode4j.ode.OdeMath.dDOT14;
...
/**
 * standard ODE geometry primitives: public API and pairwise collision
 * functions.
 * 
...
public class DxCapsule extends DxGeom implements DCapsule {
	static class CollideCapsuleBox implements DColliderFn {
		...
		public int dColliderFn(DGeom o1, DGeom o2, int flags,
			...
			return dCollideCapsuleBox((DxCapsule) o1, (DxBox) o2, flags,
					contacts, 1);
		}
		...
		int dCollideCapsuleBox(DxCapsule o1, DxBox o2, int flags,
			...
			// p1.v[2] = o1._final_posr.pos.v[2] + clen *
			// o1._final_posr.R.v[10];
			p1.eqSum(o1.final_posr().pos(), o1.final_posr().R()
					.columnAsNewVector(2), clen);
			// p2.v[0] = o1._final_posr.pos.v[0] - clen * o1._final_posr.R.v[2];
			// p2.v[1] = o1._final_posr.pos.v[1] - clen * o1._final_posr.R.v[6];
			// p2.v[2] = o1._final_posr.pos.v[2] - clen *
			// o1._final_posr.R.v[10];
			p2.eqSum(o1.final_posr().pos(), o1.final_posr().R()
					.columnAsNewVector(2), -clen);
	...
	static class CollideCapsuleCapsule implements DColliderFn {
		...
		public int dColliderFn(DGeom o1, DGeom o2, int flags,
				DContactGeomBuffer contacts) {
			return dCollideCapsuleCapsule((DxCapsule) o1, (DxCapsule) o2,
					flags, contacts, 1);
		}

		// int dCollideCapsuleCapsule (dxGeom *o1, dxGeom *o2,
		// int flags, dContactGeom *contact, int skip)
		int dCollideCapsuleCapsule(DxCapsule cyl1, DxCapsule cyl2, int flags,
				DContactGeomBuffer contacts, int skip) {
			// dIASSERT (skip >= (int)sizeof(dContactGeom));
			dIASSERT(skip == 1);
			// dIASSERT (o1.type == dCapsuleClass);
			...
			// axis1.set(cyl1._final_posr.R.get(2), cyl1._final_posr.R.get(6),
			// cyl1._final_posr.R.get(10));
			...
			// axis2.set(cyl2._final_posr.R.get(2), cyl2._final_posr.R.get(6),
			// cyl2._final_posr.R.get(10));
			DVector3C axis2 = cyl2.final_posr().R().columnAsNewVector(2);

			// if the cylinder axes are close to parallel, we'll try to detect
			// up to
			// two contact points along the body of the cylinder. if we can't
			// find any
			// points then we'll fall back to the closest-points algorithm. note
			// that
			// we are not treating this special case for reasons of degeneracy,
			// but
			// because we want two contact points in some situations. the
			// closet-points
			// algorithm is robust in all casts, but it can return only one
			// contact.
			...
			if (det < tolerance) {
				// the cylinder axes (almost) parallel, so we will generate up
				// to two
				// contacts. alpha1 and alpha2 (line position parameters) are
				// related by:
				// alpha2 = alpha1 + (pos1-pos2)'*axis1 (if axis1==axis2)
				// or alpha2 = -(alpha1 + (pos1-pos2)'*axis1) (if axis1==-axis2)
				// first compute where the two cylinders overlap in alpha1
				// space:
				if (a1a2 < 0) {
					// axis2[0] = -axis2[0];
					// axis2[1] = -axis2[1];
				...
				if (lo <= hi) {
					...
					if (num_contacts >= 2 && lo < hi) {
						// generate up to two contacts. if one of those contacts
						// is
						// not made, fall back on the one-contact strategy.
						// for (i=0; i<3; i++) sphere1[i] = pos1[i] +
						// lo*axis1[i];
						sphere1.set(axis1).scale(lo).add(pos1);
						// for (i=0; i<3; i++) sphere2[i] = pos2[i] +
						// (lo+k)*axis2[i];
						sphere2.set(axis2).scale(lo + k).add(pos2);
						int n1 = DxCollisionUtil.dCollideSpheres(sphere1,
								cyl1._radius, sphere2, cyl2._radius, contacts);
						if (n1 != 0) {
							// for (i=0; i<3; i++) sphere1[i] = pos1[i] +
							// hi*axis1[i];
							...
							int n2 = DxCollisionUtil.dCollideSpheres(sphere1,
									cyl1._radius, sphere2, cyl2._radius, c2);
							if (n2 != 0) {
								c2.get().g1 = cyl1;
								c2.get().g2 = cyl2;
								c2.get().side1 = -1;
					...
					return DxCollisionUtil.dCollideSpheres(sphere1,
							cyl1._radius, sphere2, cyl2._radius, contacts);
				}
			}
			...
			DxCollisionUtil.dClosestLineSegmentPoints(a1, a2, b1, b2, sphere1,
					sphere2);
	...
	static class CollideCapsulePlane implements DColliderFn {
		...
		public int dColliderFn(DGeom o1, DGeom o2, int flags,
				DContactGeomBuffer contacts) {
			return dCollideCapsulePlane((DxCapsule) o1, (DxPlane) o2, flags,
					contacts, 1);
		}
		...
		int dCollideCapsulePlane(DxCapsule o1, DxPlane o2, int flags,
			...
			// double sign = (dDOT14 (plane._p,0,o1._final_posr.R.v,2) > 0) ?
			// (-1.0) : (1.0);
			double sign = (planePos.dot(o1.final_posr().R().viewCol(2)) > 0) ? (-1.0)
					: (1.0);
			// p[0] = o1.final_posr.pos[0] + o1.final_posr.R[2] * ccyl.lz *
			// REAL(0.5) * sign;
			// p[1] = o1.final_posr.pos[1] + o1.final_posr.R[6] * ccyl.lz *
			// REAL(0.5) * sign;
			// p[2] = o1.final_posr.pos[2] + o1.final_posr.R[10] * ccyl.lz *
			// REAL(0.5) * sign;
			DVector3C p = o1.final_posr().R().columnAsNewVector(2);
			// p.set(o1._final_posr.R.get(2), o1._final_posr.R.get(6),
			// o1._final_posr.R.get(10));
			...
			if ((flags & NUMC_MASK) >= 2) {
				...
				// p.set(0, o1.final_posr.pos[0] - o1.final_posr.R[2] * ccyl.lz
				// * REAL(0.5) * sign );
				// p.set(1, o1.final_posr.pos[1] - o1.final_posr.R[6] * ccyl.lz
				// * REAL(0.5) * sign );
				// p.set(2, o1.final_posr.pos[2] - o1.final_posr.R[10] * ccyl.lz
				// * REAL(0.5) * sign );
	...
	static class CollideCapsuleSphere implements DColliderFn {
		...
		public int dColliderFn(DGeom o1, DGeom o2, int flags,
				DContactGeomBuffer contacts) {
			return dCollideCapsuleSphere((DxCapsule) o1, (DxSphere) o2, flags,
					contacts, 1);
		}
		...
		int dCollideCapsuleSphere(DxCapsule o1, DxSphere o2, int flags,
			...
			double alpha = o1.final_posr().R().get02()
					* (o2.final_posr().pos().get0() - o1.final_posr().pos()
							.get0())
					+ o1.final_posr().R().get12()
					* (o2.final_posr().pos().get1() - o1.final_posr().pos()
							.get1())
					+ o1.final_posr().R().get22()
					* (o2.final_posr().pos().get2() - o1.final_posr().pos()
							.get2());
			...
			// p.v[2] = o1._final_posr.pos.v[2] + alpha *
			// o1._final_posr.R.v[10];
			p.eqSum(o1.final_posr().pos(), o1.final_posr().R()
					.columnAsNewVector(2), alpha);
			return DxCollisionUtil.dCollideSpheres(p, ccyl._radius, o2
					.final_posr().pos(), sphere.getRadius(), contacts);
		}
	}

	public static DxCapsule dCreateCapsule(DxSpace space, double radius,
			double length) {
																		...
																		// (space,1)
	{
		...
		_radius = __radius;
		_lz = __length;
		// updateZeroSizedFlag(!_radius/* || !_length -- zero length capsule is
		// not a zero sized capsule*/);
	...
	public double dGeomCapsulePointDepth(double x, double y, double z) {
		...
		return _radius
				- dSqrt((x - a.get0()) * (x - a.get0()) + (y - a.get1())
						* (y - a.get1()) + (z - a.get2()) * (z - a.get2()));
	}
	...
	public void dGeomCapsuleSetParams(double radius, double length) {
		...
		_radius = radius;
		_lz = length;
		// updateZeroSizedFlag(!radius/* || !length -- zero length capsule is
		// not a zero sized capsule*/);
