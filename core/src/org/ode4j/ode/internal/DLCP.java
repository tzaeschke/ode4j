...
//		dMatrixN A3 = L.mulNew(D).mulNew( L.transposeNew() );
			...
			double[] w, int nub, double[] lo, double[] hi)
	{
		...
		DLCP lcp = newdLCP(n,0,A,x,b,w,tmp,tmp,L,d,Dell,ell,tmp,dummy,dummy,p,C,Arows);
		nub = lcp.getNub();
...
//		done: TODO if changed, change done: above
			...
			double[] w, int nub, double[] lo, double[] hi, int []findex)
	{
		...
		DLCP lcp=newdLCP(n,nub,A,x,b,w,lo,hi,L,d,Dell,ell,delta_w,state,findex,p,C,Arows);
		nub = lcp.getNub();
...
//TODO		delete lcp;
	...
	private static DLCP newdLCP(int n, int i, double[] a, double[] x, double[] b,
...
class dLCP_FAST extends DLCP {
	...
	double[] Adata,x,b,w,lo,hi;	// permuted LCP problem data
	double[] L,d;				// L*D*L' factorization of set C
	double[] Dell,ell,tmp;
	int[] state,findex,p,C;
