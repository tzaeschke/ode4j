...
import static org.ode4j.ode.internal.Misc.*;
...
// TODO CHECK-TZ REMOVE THIS FILE
...
public class DLCP_RemoveMe {
	...
	void transfer_b_to_x(double pairsbx[PBX__MAX], int n, boolean zero_b)
	{
		//dReal *const endbx = pairsbx + (size_t)n * PBX__MAX;
		int endbx = pairsbx + (size_t)n * PBX__MAX;
...
//	{ for (int i=0; i<nN; i++) p[i+nC] += s*q[i+nC]; }
	...
	//		    dReal *_Dell, dReal *_ell, dReal *_tmp,
	//		    int *_state, int *_findex, int *_p, int *_C, dReal **Arows)
	DLCP_RemoveMe(int _n, int _nskip, int _nub, double []_Adata, double[] _x, double[] _b, double[] _w,
	...
	// TZ: We stick with the old x/b and lo/hio, which is more efficient because it does not require Objects.
	...
	//	void dxSolveLCP (DxWorldProcessMemArena memarena,
	//					 int n, double[] A, double[] pairsbx, double[] w,
	//					 int nub, double[] pairslh, int[] findex)
	...
	{
		//		dAASSERT(pairsbx.length = PBX__MAX);
		//		dAASSERT(pairsbx.length = PLH__MAX);
		...
		DLCP_RemoveMe lcp = new DLCP_RemoveMe(n,nskip,nub,A,x,b,w,lo,hi,L,d,Dell,ell,delta_w,state,findex,p,C,Arows);
...
//    }
            ...
            DLCP_RemoveMe.dSolveLCP (arena, n,A2,x,b2,w,nub,lo2,hi2,null);
...
//		    }
	...
	static
	void swapProblem (double[] A, double pairsbx[PBX__MAX], double[] w, double pairslh[PLH__MAX],
...
//			dSASSERT(PBX__MAX == 2);
			dxSwap(pairsbx, i1 * PBX__MAX + PBX_B, pairsbx, i2 * PBX__MAX + PBX_B);
			dxSwap(pairsbx, i1 * PBX__MAX + PBX_X, pairsbx, i2 * PBX__MAX + PBX_X);
			dSASSERT(PBX__MAX == 2);
...
//			dSASSERT(PLH__MAX == 2);
			dxSwap(pairslh, i1 * PLH__MAX + PLH_LO, pairslh, i2 * PLH__MAX + PLH_LO);
			dxSwap(pairslh, i1 * PLH__MAX + PLH_HI, pairslh, i2 * PLH__MAX + PLH_HI);
			dSASSERT(PLH__MAX == 2);
